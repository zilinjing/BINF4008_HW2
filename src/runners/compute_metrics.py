...
def _compute_prf(gt: pd.Series, pred: pd.Series) -> Dict[str, Optional[float]]:
    """
    TODO(student-metrics-1): Compute binary classification metrics for one label.
    Expected output keys: tp, fp, fn, support, precision, recall, f1
    Hint:
    - tp: gt=1 and pred=1
    - fp: gt=0 and pred=1
    - fn: gt=1 and pred=0
    - support: number of positives in gt
    - Use None when a denominator is zero.
    """
    # START TODO(student-metrics-1)
    raise NotImplementedError(
        "TODO(student-metrics-1): implement _compute_prf(gt, pred)"
    )
    # END TODO(student-metrics-1)
    return {
        "tp": tp,
        "fp": fp,
        "fn": fn,
...
def _compute_label_metrics(
    ...
    """
    TODO(student-metrics-2): Aggregate label metrics across diseases.
    Compute:
    ...
    """
    # START TODO(student-metrics-2)
    raise NotImplementedError(
        "TODO(student-metrics-2): implement macro/micro aggregation in _compute_label_metrics(...)"
    )
    # END TODO(student-metrics-2)
