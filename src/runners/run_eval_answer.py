...
def build_report_prompt(row_prompt: str, prompt_cfg: Dict[str, Any]) -> str:
    ...
    user_prompt = str(prompt_cfg.get("user", "")).strip()
    row_prompt = str(row_prompt).strip()
    prompt_parts = [p for p in [system_prompt, user_prompt, row_prompt] if p]
    if not prompt_parts:
        return "Provide a description of the findings in the radiology image."
    return "\n\n".join(prompt_parts)
